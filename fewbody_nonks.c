...
int fb_nonks_func(double t, const double *y, double *f, void *params)
{
  ...
  double clight, clight2, clight4, clight5;
  fb_units_t units;
  ...
  for (i=0; i<nstar; i++) {
    for (j=0; j<i; j++) {
      for (k=0; k<3; k++) {
        // JMA 8-20-12 -- Although it looks like we're setting these values
        // from an uninitialized array, it turns out that it is in the
        // j=i+1 for loop below.  Tricky, tricky....
...
/*       } */
  ...
  for (i=0; i<nstar; i++) {
    ...
    for (j=0; j<i; j++) {
      for (k=0; k<3; k++) {
        /* Do you really mean to divide by SM here?  Shouldn't you do this
           above where you set FB_REL()?  There could be a problem here for
           non-equal masses.  */
...
/*           R = fb_mod(r); */
/*     for (k=0; k<3; k++) { */
/*       f[i*6+k+3] -= Mclus * y[i*6+k]/(fb_cub(R)*sqrt(fb_cub(1.0+fb_sqr(aclus/R)))); */
/*     } */
/*   } */
  /* DEBUG */
