...
void print_usage(FILE *stream)
{
  ...
  fprintf(stream, "  -m --m000 <m000/MSUN>        : set masses of the five stars: [m0 m1] [[m00 m01] m1] [%.6g]\n", FB_M000/FB_CONST_MSUN);
  fprintf(stream, "  -r --r1 <r1/R_SUN>           : set merge radius of interloper [%.6g]\n", FB_R1/FB_CONST_RSUN);
  ...
  fprintf(stream, "  -b --bmax <b/AU>             : set maximum impact parameter [%.6g]\n", FB_BMAX/FB_CONST_AU);
  ...
  fprintf(stream, "  -E --nexp                    : set number of experiments [%d]\n", FB_NEXP);
...
int main(int argc, char *argv[])
{
  ...
  double m000, m001, m01, m1, r000, r001, r01, r1, a00, a0, e00, e0, inc, peri_in;
  double peri_out, inc_in, inc_out;
  double bmax, vinf, b, rtid;
  double masses[5];
  int index, mcount;
  char *next;
  int nexp;
  ...
  const char *short_opts = "m:r:H:C:G:Q:g:i:a:q:e:F:p:B:I:b:v:E:t:D:c:A:R:N:O:z:x:y:P:Q:S:T:U:k:s:dVh";
  const struct option long_opts[] = {
    {"m000", required_argument, NULL, 'm'},
    {"r1", required_argument, NULL, 'r'},
  ...
  while ((i = getopt_long(argc, argv, short_opts, long_opts, NULL)) != -1) {
    ...
    case 'm':
      ...
      while (index < argc) {
        ...
        index++;
        if (next[0] != '-') {
          masses[mcount++] = atof(next) * FB_CONST_MSUN;
        }
        else break;
      }
      optind = index - 1;
    ...
    case 'I':
      ...
      break;
    case 'b':
      bmax = atof(optarg) * FB_CONST_AU;
      if (bmax < 0) {
        fprintf(stderr, "bmax must be positive.\n");
        return(1);
      }
      break;
    case 'v':
      vinf = atof(optarg) * FB_CONST_KMS;
      if (vinf < 0) {
    ...
    case 'E':
      nexp = atoi(optarg);
      if (nexp < 0) {
        fprintf(stderr, "nexp must be positive.\n");
        return(1);
      }
      break;
    case 't':
  ...
  for (k=0; k<nexp; k++) {
    ...
    if (peri_in == -1.) {
      peri_in = 2 * FB_CONST_PI * gsl_rng_uniform(rng);
    }
    if (peri_out == -1.) {
      peri_out = 2 * FB_CONST_PI * gsl_rng_uniform(rng);
    }
    ...
    fprintf(stderr, "  vinf=%.6g km/s  bmax=%.6g AU\n", vinf/FB_CONST_KMS, bmax/FB_CONST_AU);
    ...
    */
    ...
    hier.obj[1] = &(hier.hier[hier.hi[1]+3]);
    hier.obj[2] = NULL;
    hier.obj[3] = NULL;
    ...
    b = sqrt(gsl_rng_uniform(rng)) * bmax;
		...
		rtid = pow(2.0*(hier.hier[hier.hi[1]+3].m + hier.hier[hier.hi[3]].m) / \
