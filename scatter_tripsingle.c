...
void print_usage(FILE *stream)
{
  ...
  fprintf(stream, "  -M --m1 <m0/MSUN>            : set mass of interloper [%.6g]\n", FB_M1/FB_CONST_MSUN);
...
int main(int argc, char *argv[])
{
  ...
  const char *short_opts = "m:n:o:M:r:H:C:G:Q:g:i:a:q:e:F:p:B:I:b:X:v:t:D:c:A:R:N:O:z:x:y:P:Q:S:T:U:k:s:dVh";
  const struct option long_opts[] = {
    {"m000", required_argument, NULL, 'm'},
    {"m001", required_argument, NULL, 'n'},
    {"m01", required_argument, NULL, 'o'},
    {"m1", required_argument, NULL, 'M'},
  ...
  */
  ...
  vcrit = sqrt(FB_CONST_G * (a0 * m000 * m001 + a00 * (m000 + m001) * m01) * (m000 + m001 + m01 + m1) /
    (a0 * a00 * (m000 + m001 + m01) * m1)) / units.v;
  if (input_bmax == -1) {
    bmax = (4 / (vinf / vcrit) + 3) * a0 * xbmax;
  } else {
    bmax = input_bmax;
  }
