...
int main(int argc, char *argv[])
{
  ...
  const char *short_opts = "m:n:r:g:a:e:t:D:c:A:R:N:z:y:x:P:Q:S:T:U:k:s:dVh";
...
// JMA 5/9/12 -- These two lines were commented out originally.  This means
// that you cannot change the radius.
//  hier.hier[hier.hi[1]+0].R = r00;
//  hier.hier[hier.hi[1]+1].R = r01;
  
// JMA 5/9/12
// These two lines define the radius of the two objects to be 3.25 times
// the Schwarzschild radius.  (This is apparently to keep the orbital
// velocity smaller than 0.4c so that the Post-Newtonian equations remain
// valid.  This should not much matter when calculating the orbital decay
// time since the merger time is basically instantaneous at 3.25 times the
// Schwarzschild radius.
  hier.hier[hier.hi[1]+0].R = FB_REFF_BH*FB_CONST_G*m00/(FB_CONST_C*FB_CONST_C);
  hier.hier[hier.hi[1]+1].R = FB_REFF_BH*FB_CONST_G*m01/(FB_CONST_C*FB_CONST_C);
